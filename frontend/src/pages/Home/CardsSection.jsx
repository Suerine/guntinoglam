...
const cards = [
  { id: 1, image: exclusiveCardImage1, title: 'Faransawi', description: 'EXPLORE' },
  { id: 2, image: exclusiveCardImage2, title: 'Maqbal', description: 'EXPLORE' },
  { id: 3, image: exclusiveCardImage3, title: 'Guntino', description: 'EXPLORE' },
]
...
const CardsSection = ({ isMobile }) => {
  if (isMobile) {
    return cards.map((card) => (
      ...
      >
        <Link to={`/products/${card.id}`} className="block w-full h-full">
  ...
  return (
    <div className="flex w-full gap-1" style={{ height: '100vh', scrollSnapAlign: 'start', scrollSnapStop: 'always', flexShrink: 0, background: '#FFF5FF' }}>
      {cards.map((card) => (
        <Link key={card.id} to={`/products/${card.id}`} className="relative flex-1 overflow-hidden group">
          <img src={card.image} alt={card.title} className="w-full h-full object-cover" />
          <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent" />
          <div className="absolute bottom-6 right-6 text-right">
            <h2 className="text-white text-xl font-light tracking-widest">{card.title}</h2>
